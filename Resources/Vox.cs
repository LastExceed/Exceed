...
namespace Resources {
    public class Vox {
        ...
        public class Model {
            public ByteVector size;
            public List<Voxel> voxels = new List<Voxel>();

            public Model(ByteVector size) => this.size = size;
        }
            ...
            while (fileStream.Position < fileStream.Length) {
                ...
                switch (chunkName) {
                    case "MAIN":
                        ...
                        break;
                    case "PACK":
                        //models = new Model[reader.ReadInt32()];
                        break;
                    case "SIZE":
                        currentModel++;
                        var size = new ByteVector() {
                            x = (byte)reader.ReadInt32(),
                            y = (byte)reader.ReadInt32(),
                            z = (byte)reader.ReadInt32(),
                        };
                        models.Add(new Model(size));
                        break;
                    case "XYZI":
                        var voxelCount = reader.ReadInt32();
                        for (int i = 0; i < voxelCount; i++) {
                            models[currentModel].voxels.Add(new Voxel() {
                                position = new IntVector() {
                                    x = reader.ReadByte(),
                                    y = reader.ReadByte(),
                                    z = reader.ReadByte(),
                                },
                                colorIndex = (byte)(reader.ReadByte() - 1),
                            });
                        }
                        break;
                    case "RGBA":
                        colorPalette = new uint[256];
                        for (int i = 0; i < 256; i++) {
                            colorPalette[i] = reader.ReadUInt32();
                        }
                        break;
                    case "MATL":
                        var material = new Material() {
                            id = reader.ReadInt32(),
                            type = (Material.Type)reader.ReadInt32(),
                            weight = reader.ReadSingle(),
                        };
                        var propertyBits = reader.ReadInt32(); //bitfield
                        if (Bit.GetBit(propertyBits, 0))
                            material.plastic = reader.ReadSingle();
                        if (Bit.GetBit(propertyBits, 1))
                            material.roughness = reader.ReadSingle();
                        if (Bit.GetBit(propertyBits, 2))
                            material.specular = reader.ReadSingle();
                        if (Bit.GetBit(propertyBits, 3))
                            material.IOR = reader.ReadSingle();
                        if (Bit.GetBit(propertyBits, 4))
                            material.attenuation = reader.ReadSingle();
                        if (Bit.GetBit(propertyBits, 5))
                            material.power = reader.ReadSingle();
                        if (Bit.GetBit(propertyBits, 6))
                            material.glow = reader.ReadSingle();
                        material.isTotalPower = Bit.GetBit(propertyBits, 7);
                    ...
                    case "nTRN":
                        ...
                        foreach (var voxel in models[nTRNcount].voxels) {
                            voxel.position.x += x;
                            voxel.position.y += y;
                            voxel.position.z += z;
                        }
        ...
        public List<Packet.ServerUpdate.BlockDelta> Parse() {
            var parsedModel = new List<Packet.ServerUpdate.BlockDelta>();
            foreach (var model in models) {
                foreach (var voxel in model.voxels) {
                    var colorBytes = BitConverter.GetBytes(colorPalette[voxel.colorIndex]);
                    parsedModel.Add(new Packet.ServerUpdate.BlockDelta {
                        ...
                        type = colorBytes[2] == 255 && colorBytes[1] == 0 && colorBytes[0] == 0 ? BlockType.Liquid : BlockType.Solid,
                    });
                }
            }
            return parsedModel;
